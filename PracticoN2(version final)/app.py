        ...
        if not paquetes_seleccionados:
            ...
            try:
                ...
                if not nro_transporte:
                    ...
                    nro_transporte=nro_transporte.numerotransporte+1
                ...
                for nro_envio in paquetes_seleccionados:
                    if nro_envio:
                        paquete_a_modificar=Paquete.query.filter(Paquete.numeroEnvio == nro_envio).first()#Segun el numero de envio del paquete que selecciono el despachador se modifica el id del trasnporte por el nuevo transporte
                        paquete_a_modificar.idtransporte = transporte.id
                        
                        
                print(f"Objetos modificados: {db.session.dirty}")#Muestra los paquetes a los que se le modifico el id de transporte, pero no se modifica el id de la sucursal pues todavia no llego a destino
                db.session.commit()
                suc = Sucursal.query.filter(Sucursal.id == transporte.idsucursal).first()
                return render_template('ExitoSalidaTransporte.html',mensaje='Los paquetes se encuentran en camino, se dirige a la sucursal nro :',nro_sucursal=suc.numero, nro_transporte = nro_transporte)
            except Exception as e:
...
def RegistrarSalidaTransporte():
    if request.method == 'POST':
        ...
        sucursal = request.form['sucursal_destino']#Sucursal donde ira el transporte
        if sucursal:
...
def LLegada_transporte():
    if request.method == 'POST':
        if not request.form['transporte']:
            ...
            try:
                idtransporte = request.form.get('transporte') 
                print(f"id transporte: {idtransporte}")
                ...
                if not paquetes:
                    ...
                    #Se modifca el id de los paquetes con la sucursal destino
                    ...
                    #Buscamos el transporte que llego a la sucursal y colocamos la hora de llegada
                    transporte = Transporte.query.filter(Transporte.id == idtransporte).first()
                    transporte.fechahorallegada = llegada_del_transporte
                    print(f"Objetos modificados: {db.session.dirty}")
                    db.session.commit()
...
#Pendiente 
""""
Combinar los htmls de exitos y errores en uno de error y en otro de exito para reducir la cantidad de templates
Ver manera de que no muestre los segundo en la fecha de salida y transporte en la tabla de transporte
Eliminar los css extras, solo dejar uno con un estilo,meter bootstrap para mejorar el diseño
Verficar que toda mal entrada de datos es resuelta
