...
class JuegoSimon(VentanaInicio):
    def __init__(self):
       ...
       #ventana
        ...
        #menu
        BarraPuntaje=Menu(self.root)
        menuPuntaje=Menu(BarraPuntaje,tearoff=1)
        menuPuntaje.add_command(label='Ver puntajes',command=self.VentanaPuntajes)
        menuPuntaje.add_command(label='Salir',command=self.Salir)
        BarraPuntaje.add_cascade(label="Puntajes", menu=menuPuntaje)
        self.root.config(menu=BarraPuntaje)
        # Lista de botones
        self.__botones = []
        ...
        self.botonSTART = Button(self.canvas, text="START")
        self.canvas.create_window(150, 270, window=self.botonSTART)
        
        self.botonSalir = Button(self.canvas, text="SALIR",command=self.Salir)
        self.canvas.create_window(100, 270, window=self.botonSalir)
    
    def setControlador(self, controlador):
        self.controlador = controlador
        self.botonSTART.config(command=controlador.agregarJugador)
    ...
    def secuenciaAleatoria(self):
        ...
        for indice,color in enumerate(self.__listaSecuenciaRandom):#toma indice y valor de la lista
            ...
            self.canvas.after(indice*1000, lambda b=boton,c=color: self.cambiarColorBoton(b, c))
    ...
    def gameOver(self):
        self.controlador.ActualizaPuntuacion_GameOver(self.__puntaje) Si lo dejo cada vez
        messagebox.showinfo("GAME OVER", "Puntaje obtenido: {}".format(self.__puntaje))
        self.__listaSecuenciaRandom = []
        self.__secuenciaJugador = []
        self.__puntaje = 0
    def VentanaPuntajes(self):
        ventana = Toplevel()
        ventana.geometry("400x250")
        ventana.resizable(width=False, height=False)
        ventana.title("Galería de Puntajes")
        
        # Marco para la tabla
        marcoTabla = Frame(ventana)
        marcoTabla.pack(fill='both', expand=True, padx=10, pady=10)

        # Treeview para mostrar los puntajes
        columnas = ("Jugador", "Fecha", "Hora", "Puntaje")
        tree = ttk.Treeview(marcoTabla, columns=columnas, show='headings')
        
        for col in columnas:
            tree.heading(col, text=col)
            tree.column(col, minwidth=0, width=80, stretch=tk.NO)
        
        # Ejemplo de datos
        #Ordena los datos
        self.controlador.getListaPuntajesOrdenados()
        for fila in self.controlador.getListaManejador():
            tree.insert("", tk.END, values=fila)
        
        tree.pack(fill='both', expand=True)

        # Botón para cerrar la ventana
        marcoBoton = Frame(ventana)
        marcoBoton.pack(fill='x', padx=10, pady=5)
        botonCerrar = tk.Button(marcoBoton, text="Cerrar", command=ventana.destroy)
